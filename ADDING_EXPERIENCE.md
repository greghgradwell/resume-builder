...
The master resume in `data/resume.yaml` has a consistent bullet style. This workflow extracts raw experience through conversation, then translates it into bullets that match that style — without embellishment.
...
- name: Company Name
  ...
  bullets:
    - id: a1b2c3d4            # generate with: python -c "import uuid; print(uuid.uuid4().hex[:8])"
